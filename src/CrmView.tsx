...
import { api } from "./api";
import type { ContractDetail, CustomerDetail, CustomerSummary, TariffDocument } from "./types";
...
export function CrmView() {
  ...
  const [contractDocuments, setContractDocuments] = useState<TariffDocument[]>([]);
  ...
  useEffect(() => {
    ...
    setContract(null); setContractDocuments([]);
  ...
  const openContract = async (contractId: string) => {
    const [detail, documents] = await Promise.all([api.contract(contractId), api.contractDocuments(contractId)]);
    setContract(detail); setContractDocuments(documents);
  };
      ...
      <div className="customer-detail">
        {!selected ? <div className="customer-empty"><CircleUserRound /><h3>Kundenakte auswählen</h3></div> : <>
          ...
          <div className="customer-grid">
            ...
            <section className="crm-card contracts-card"><header><FileKey2 /><h3>Verträge</h3><span>{selected.contracts.length}</span></header><div className="contract-list">{selected.contracts.map((item) => <button key={item.contractId} className={contract?.contractId === item.contractId ? "active" : ""} onClick={() => void openContract(item.contractId)}><span className={`contract-line ${item.line.toLowerCase()}`}>{item.line}</span><span><strong>{item.productName}</strong><small>{item.contractId} · {item.tariffGenerationId}</small><em>{money(item.annualPremium, item.currency)} p. a. · Summe {money(item.insuredSum, item.currency)}</em></span><span className={`contract-status ${item.status === "AKTIV" ? "active" : ""}`}>{item.status}</span><ChevronRight /></button>)}</div>
              {contract && <div className="contract-inspector"><header><div><span>Vertragsdetails</span><strong>{contract.contractId}</strong></div><button onClick={() => { setContract(null); setContractDocuments([]); }}>Schließen</button></header><div className="contract-facts"><span><small>Beginn</small><strong>{shortDate(contract.startDate)}</strong></span><span><small>Zahlweise</small><strong>{contract.paymentFrequency}</strong></span><span><small>Kanal</small><strong>{contract.channel}</strong></span><span><small>Sachbearbeitung</small><strong>{contract.handlerId || "–"}</strong></span></div><h4>Passendes Bedingungswerk</h4>{contractDocuments.map((document) => <a className="contract-document" key={document.id} href={`/api/tariffs/${document.id}/download`}><FileKey2 /><span><strong>{document.id}</strong><small>{document.revision} · synthetisches Workshop-PDF</small></span><ChevronRight /></a>)}<h4>Deckungen</h4>{contract.coverages.map((coverage) => <div className="coverage-row" key={coverage.id}><span><strong>{coverage.component || coverage.type}</strong><small>{coverage.id}</small></span><em>{coverage.sum != null ? money(coverage.sum, contract.currency) : "eingeschlossen"}{coverage.deductible != null ? ` · SB ${money(coverage.deductible, contract.currency)}` : ""}</em></div>)}</div>}
            </section>
